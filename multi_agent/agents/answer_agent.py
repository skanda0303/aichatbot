...
def _get_system_prompt() -> str:
    ...
    return (
        ...
        "SYNTHESIS RULES:\n"
        "1. Answer directly from facts in the provided context — never tell the user to visit a site.\n"
        "2. Be comprehensive and detailed. Use bullet points or bold text where helpful.\n"
        "3. Trust provided context over your training memory.\n"
        "4. CHRONOLOGICAL TIMELINES: Verify dates carefully and list oldest to newest.\n"
        "5. If both RAG and Web context are provided, prefer RAG content and supplement "
        "with Web only where RAG has gaps.\n"
        "6. Cite sources naturally inline (e.g. 'According to the documentation...') "
        "when using web sources.\n"
        "7. STRICT TEMPORAL FILTERING: For questions asking for 'next', 'upcoming', or 'future' events/dates, "
        f"strictly EXCLUDE any event whose date occurred on or before today ({now_str}). "
        "Never list past events as 'next' or 'upcoming', even with a disclaiming note that they have passed."
    )
